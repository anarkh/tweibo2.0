...
class OAuth
{
    public static $client_id = '801323796';
    public static $client_secret = '156f7633ab8626e9a6fa23e9ab201e13';
     ...
     * 验证授权是否有效
     */
    public static function checkOAuthValid($memberId)
    {
        $r = json_decode(Tencent::api($_SESSION[$memberId]['t_openid'],'user/info'), true);
        if ($r['data']['name']) {
            return true;
        } else {
            self::clearOAuthInfo();
...
class Tencent
{
     ...
     * @return string
     */
    public static function api($memberArr,$command, $params = array(), $method = 'GET', $multi = false)
    {
        if (isset($memberArr['t_openid'])) {//OAuth 2.0 方式
            //鉴权参数            
            $params['access_token'] = $memberArr['t_access_token'];
            $params['oauth_consumer_key'] = OAuth::$client_id;
            $params['openid'] = $memberArr['t_openid'];
            $params['oauth_version'] = '2.a';
            $params['clientip'] = Commons::getClientIp();
            $params['scope'] = 'all';
        ...
        } elseif (isset($memberArr['t_openid']) && isset($memberArr['t_openkey'])) {//openid & openkey方式
            $params['appid'] = OAuth::$client_id;
            $params['openid'] = $memberArr['t_openid'];
            $params['openkey'] = $memberArr['t_openkey'];
            $params['clientip'] = Commons::getClientIp();
            $params['reqtime'] = time();
            $params['wbversion'] = '1';
